...
import { isErrnoException, type TranslationsData } from "./types";
  ...
  try {
    ...
    if (isErrnoException(error)) {
      switch (error.code) {
        ...
        default:
          console.error(
            `An unknown error occurred while reading file: ${error.message}`
          );
      }
    }
  }
  ...
  try {
    ...
    if (isErrnoException(error)) {
      switch (error.code) {
        ...
        case "EACCES":
          console.error(
            `Permission denied while accessing directory: ${folderPath}.`
          );
          break;
        case "ENOTDIR":
          console.error(
            `The specified path is not a directory: ${folderPath}.`
          );
          break;
        default:
          console.error(
